    ...
    <div class="content mt-3">
      <div class="col-lg-12">
        <form action="ganti.php" method="post">
          ...
          $data = mysqli_query($link,"SELECT * FROM people WHERE username = '$username'");
          while($r = mysqli_fetch_array($data)){
           ?>
          <input type="text" name="username1" value="<?php echo $r['username']; ?>"> <br><br>
          ...
          <input type="submit" name="submit" value="Ganti Password" class="btn btn-outline-primary btn-lg">
          <br><br>
          <hr>
          <?php
          if(isset($_POST['submit'])){
            ...
            // $kode   = $_SESSION['level']; //ini dari session saat login
            $cek      = mysqli_query($link, "SELECT * FROM people WHERE username ='$username1' AND password ='$password_lama'");
            ...
            if($cek->num_rows){
              ...
              if($password_baru == $konfirmasi_password){
                ...
                $update     = mysqli_query($link, "UPDATE people SET password='$password_baru' WHERE username='$username1'");
                if($update){
